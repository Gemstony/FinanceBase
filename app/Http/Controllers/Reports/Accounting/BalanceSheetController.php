...
use App\Exports\Reports\Accounting\BalanceSheetExport;
use App\Http\Controllers\Controller;
use App\Models\AccountClass;
use App\Models\JournalEntryLines;
use App\Models\SubShop;
use App\Services\Reports\Accounting\BalanceSheetService;
...
class BalanceSheetController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $compareAsOf = $request->date('compare_as_of');

        $filters = [
            'as_of' => $asOf,
            'compare_as_of' => $compareAsOf ? Carbon::parse((string) $compareAsOf) : null,
        ...
        $commonParams = array_filter([
            ...
            'compare_as_of' => $compareAsOf ? Carbon::parse((string) $compareAsOf)->toDateString() : null,
            'subshop_id' => $subshopId,
            'account_class_id' => $filters['account_class_id'],
        ], fn ($v) => $v !== null && $v !== '');

        return view('reports.accounting.balance_sheet', [
            'asOf' => $asOf->toDateString(),
            'compareAsOf' => $compareAsOf ? Carbon::parse((string) $compareAsOf)->toDateString() : null,
            'subshops' => $allSubshops,
            'selectedSubshopId' => $subshopId,
            'accountClasses' => $accountClasses,
            'report' => $report,
            'exportUrl' => route('reports.accounting.balance_sheet.export', array_merge(['format' => 'xlsx'], $commonParams)),
            'pdfUrl' => route('reports.accounting.balance_sheet.export', array_merge(['format' => 'pdf'], $commonParams)),
        ]);
    }
    ...
    public function export(Request $request, string $format = 'xlsx')
    {
        ...
        $filenameBase = 'balance-sheet-' . now()->format('Y-m-d-His');
        ...
        if (strtolower($format) === 'pdf') {
            ...
            $pdf = Pdf::loadView('reports.pdf.accounting.balance_sheet', [
                'report' => $report,
                'shop' => $shop,
        ...
        $export = new BalanceSheetExport($report);
