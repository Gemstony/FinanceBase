...
use App\Exports\Reports\Accounting\ExpensesSummaryExport;
...
{
    public function __construct(private readonly ExpensesSummaryService $service)
    {
    }
    ...
    public function index(Request $request)
    {
        ...
        $filters = [
            ...
            'expense_account_id' => $request->integer('expense_account_id') ?: null,
        ];
        ...
        $expenseAccounts = ChartsOfAccount::query()
            ...
            ->where(function ($w) {
                ...
                ->whereRaw("UPPER(ac.name) = 'EXPENSE'");
            })
        ...
        return view('reports.accounting.expenses_summary', [
            'subshops' => $allSubshops,
            'selectedSubshopId' => $subshopId,
            'accountGroups' => $accountGroups,
            'selectedAccountGroupId' => $filters['account_group_id'],
            'expenseAccounts' => $expenseAccounts,
            'selectedExpenseAccountId' => $filters['expense_account_id'],
            'report' => $report,
            'dateFrom' => $dateFrom->toDateString(),
            'dateTo' => $dateTo->toDateString(),
            'exportUrl' => route('reports.accounting.expenses_summary.export', array_merge(['format' => 'xlsx'], $commonParams)),
            'pdfUrl' => route('reports.accounting.expenses_summary.export', array_merge(['format' => 'pdf'], $commonParams)),
        ]);
    }
    ...
    public function export(Request $request, string $format = 'xlsx')
    {
        ...
        $filenameBase = 'expenses-summary-' . now()->format('Y-m-d-His');
        ...
        if (strtolower($format) === 'pdf') {
            ...
            $pdf = Pdf::loadView('reports.pdf.accounting.expenses_summary', [
                'report' => $report,
                'shop' => $shop,
        ...
        $export = new ExpensesSummaryExport($report);
