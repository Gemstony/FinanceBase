...
use App\Exports\Reports\Customers\CustomerPerformanceExport;
use App\Models\SubShop;
use App\Services\Reports\Customers\CustomerPerformanceService;
use Barryvdh\DomPDF\Facade\Pdf;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Maatwebsite\Excel\Facades\Excel;

class CustomerPerformanceController extends \App\Http\Controllers\Controller
{
    public function __construct(
        private readonly CustomerPerformanceService $service,
    ) {
    }
    ...
    public function index(Request $request)
    {
        ...
        $filters = [
            ...
            'performance_level' => $request->query('performance_level'),
        ...
        return view('reports.customers.customer_performance', [
            'subshops' => $allSubshops,
            'selectedSubshopId' => $subshopId,
            'loanProducts' => $loanProducts,
            'filters' => $filters,
            'report' => $data,
            'exportUrl' => route('reports.customers.customer_performance.export', array_merge($exportQuery, ['format' => 'xlsx'])),
            'pdfUrl' => route('reports.customers.customer_performance.export', array_merge($exportQuery, ['format' => 'pdf'])),
        ]);
    }
    ...
    public function export(Request $request, string $format = 'xlsx')
    {
        ...
        $filenameBase = 'customer-performance-report-' . now()->format('Y-m-d-His');
        ...
        if (strtolower($format) === 'pdf') {
            ...
            $pdf = Pdf::loadView('reports.pdf.customers.customer_performance', [
                'report' => $data,
        ...
        $subshopName = $subshopId ? (optional($allSubshops->firstWhere('id', $subshopId))->name) : 'All Branches';
        $export = new CustomerPerformanceExport($data, $subshopName);
