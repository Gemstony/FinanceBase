...
use App\Exports\Reports\LoanAgingInstallmentExport;
use App\Models\Customers;
use App\Models\LoanProducts;
use App\Models\SubShop;
use App\Models\User;
use App\Services\Reports\LoanAgingInstallmentReportService;
...
class LoanAgingInstallmentReportController extends \App\Http\Controllers\Controller
{
    public function __construct(
        private readonly LoanAgingInstallmentReportService $service,
    ) {
    }
    ...
    public function index(Request $request)
    {
        ...
        $customers = Customers::query()
            ->whereIn('subshop_id', $subshopId ? [$subshopId] : ($accessibleSubshopIds ?: [-1]))
        ...
        return view('reports.installment_aging', [
            ...
            'filters' => $filters,
            'report' => $report,
            'exportUrl' => route('reports.installment_aging.export', array_merge(['format' => 'xlsx'], $commonParams)),
            'pdfUrl' => route('reports.installment_aging.export', array_merge(['format' => 'pdf'], $commonParams)),
        ]);
    }
    ...
    public function export(Request $request, string $format = 'xlsx')
    {
        ...
        $filenameBase = 'installment-aging-report-' . now()->format('Y-m-d-His');
        ...
        $export = new LoanAgingInstallmentExport($report);
