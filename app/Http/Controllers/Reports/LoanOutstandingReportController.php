...
use App\Exports\Reports\LoanOutstandingExport;
use App\Models\Customers;
use App\Models\LoanProducts;
use App\Models\SubShop;
use App\Models\User;
use App\Services\Reports\LoanOutstandingReportService;
...
class LoanOutstandingReportController extends \App\Http\Controllers\Controller
{
    public function __construct(
        private readonly LoanOutstandingReportService $service,
    ) {
    }
    ...
    public function index(Request $request)
    {
        ...
        $data = $this->service->build($filters, $accessibleSubshopIds, (int) $request->integer('per_page', 50));
        ...
        return view('reports.loan_outstanding', [
            ...
            'filters' => $filters,
            'report' => $data,
            'exportUrl' => route('reports.loan_outstanding.export', array_filter([
                'format' => 'xlsx',
            ...
            'pdfUrl' => route('reports.loan_outstanding.export', array_filter([
                'format' => 'pdf',
                'as_at_date' => $asAtDate->toDateString(),
                'subshop_id' => $subshopId,
                'loan_product_id' => $filters['loan_product_id'],
    ...
    public function export(Request $request, string $format = 'xlsx')
    {
        ...
        $data = $this->service->build($filters, $accessibleSubshopIds, 5000, paginate: false);

        $filenameBase = 'loan-outstanding-report-' . now()->format('Y-m-d-His');
        ...
        $export = new LoanOutstandingExport($data);
