...
class ExpensesSummaryService
{
    public function build(array $filters, array $accessibleSubshopIds): array
    {
        ...
        $expenseAccountId = isset($filters['expense_account_id']) && $filters['expense_account_id'] ? (int) $filters['expense_account_id'] : null;

        $currentRows = $this->expenseAccountTotals($subshopIds, $fromDate, $toDate, $accountGroupId, $expenseAccountId);
        ...
        if ($fromDate && $toDate) {
            ...
            $previousRows = $this->expenseAccountTotals($subshopIds, $prevFrom, $prevTo, $accountGroupId, $expenseAccountId);
        }

        $merged = $this->mergeWithPrevious($currentRows, $previousRows);
        $tree = $this->buildTree($merged);

        $totalExpenses = round(array_sum(array_map(static fn ($r) => (float) ($r['amount'] ?? 0), $merged)), 2);
        $tree = $this->applyPercentages($tree, $totalExpenses);

        $top = $this->topExpenses($merged, 5);
        if ($totalExpenses > 0) {
            foreach ($top as $idx => $t) {
                $top[$idx]['percentage'] = round((((float) ($t['amount'] ?? 0)) / $totalExpenses) * 100, 2);
            }
        }
        $monthly = $this->monthlyTrend($subshopIds, $fromDate, $toDate, $accountGroupId, $expenseAccountId);
        ...
        $differenceTotal = round($totalExpenses - $previousTotal, 2);
        ...
        return [
            ...
            'totals' => [
                ...
                'difference_total_expenses' => $differenceTotal,
            ],
            'top_expenses' => $top,
    ...
    private function expenseAccountTotals(array $subshopIds, ?Carbon $fromDate, ?Carbon $toDate, ?int $accountGroupId, ?int $expenseAccountId): array
    {
        $q = DB::table('journal_entry_lines as jel')
            ...
            ->when($expenseAccountId !== null, fn ($qq) => $qq->where('coa.id', $expenseAccountId))
    ...
    private function applyPercentages(array $tree, float $totalExpenses): array
    {
        if ($totalExpenses <= 0) {
            return $tree;
        }
        ...
        foreach ($tree as $gk => $g) {
            foreach (($g['accounts'] ?? []) as $idx => $acc) {
                ...
                $pct = round(($amt / $totalExpenses) * 100, 2);
    ...
    private function topExpenses(array $rows, int $limit): array
    {
        $sorted = $rows;
    ...
    private function monthlyTrend(array $subshopIds, ?Carbon $fromDate, ?Carbon $toDate, ?int $accountGroupId, ?int $expenseAccountId): array
    {
        if (!$fromDate || !$toDate) {
            return [];
        }
        ...
        $q2 = DB::table('journal_entry_lines as jel')
            ...
            ->selectRaw('SUM(jel.debit - jel.credit) as amount')
            ->groupBy('month')
            ->orderBy('month')
            ->get();
