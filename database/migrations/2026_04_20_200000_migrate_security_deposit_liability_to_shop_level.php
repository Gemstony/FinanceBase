...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (!Schema::hasColumn('security_deposit_liability_accounts', 'shop_id')) {
            Schema::table('security_deposit_liability_accounts', function (Blueprint $table) {
        ...
        DB::statement('
            UPDATE security_deposit_liability_accounts sdla
            JOIN sub_shops ss ON sdla.subshop_id = ss.id
            SET sdla.shop_id = ss.shop_id
        ');
        ...
        $hasSubshopFk = !empty(DB::select(
            'SELECT 1
             ...
             LIMIT 1',
            [$databaseName, 'security_deposit_liability_accounts', 'subshop_id']
        ));
        if ($hasSubshopFk) {
            Schema::table('security_deposit_liability_accounts', function (Blueprint $table) {
        ...
        try {
            DB::statement('ALTER TABLE security_deposit_liability_accounts DROP INDEX IF EXISTS security_deposit_liability_accounts_subshop_id_unique');
        } catch (\Exception $e) {
        }

        // Drop subshop_id column (only if still exists)
        if (Schema::hasColumn('security_deposit_liability_accounts', 'subshop_id')) {
            Schema::table('security_deposit_liability_accounts', function (Blueprint $table) {
        ...
        $hasShopUnique = !empty(DB::select(
            'SELECT 1
             ...
             WHERE TABLE_SCHEMA = ?
               ...
               AND INDEX_NAME = ?
             LIMIT 1',
            [$databaseName, 'security_deposit_liability_accounts', 'security_deposit_liability_accounts_shop_id_unique']
        ));
        if (!$hasShopUnique) {
            Schema::table('security_deposit_liability_accounts', function (Blueprint $table) {
    ...
    public function down(): void
    {
        ...
        DB::statement('
            ...
            JOIN (
                ...
                FROM sub_shops 
                GROUP BY shop_id
            ) ss ON sdla.shop_id = ss.shop_id
            SET sdla.subshop_id = ss.first_subshop_id
        ');
        ...
        $hasShopFk = !empty(DB::select(
            'SELECT 1
             ...
             WHERE TABLE_SCHEMA = ?
               ...
               AND REFERENCED_TABLE_NAME IS NOT NULL
             LIMIT 1',
            [$databaseName, 'security_deposit_liability_accounts', 'shop_id']
        ));
        if ($hasShopFk) {
            Schema::table('security_deposit_liability_accounts', function (Blueprint $table) {
        ...
        try {
            DB::statement('ALTER TABLE security_deposit_liability_accounts DROP INDEX IF EXISTS security_deposit_liability_accounts_shop_id_unique');
        } catch (\Exception $e) {
        }
        ...
        $hasSubshopUnique = !empty(DB::select(
            'SELECT 1
             ...
             WHERE TABLE_SCHEMA = ?
               ...
               AND INDEX_NAME = ?
             LIMIT 1',
            [$databaseName, 'security_deposit_liability_accounts', 'security_deposit_liability_accounts_subshop_id_unique']
        ));
        if (!$hasSubshopUnique) {
            Schema::table('security_deposit_liability_accounts', function (Blueprint $table) {
