...
<head>
    <meta charset="UTF-8">
    <title>{{ ($subshop->name ?? 'Shop') }} Smart Alerts</title>
    <style>
        ...
        .badge{ display:inline-block; padding:2px 6px; font-size:7px; font-weight:700; border-radius:3px; text-transform:uppercase; color:#fff; }
        .badge-critical{ background:#d32f2f; }
        .badge-high{ background:#ff9800; }
        .badge-medium{ background:#607d8b; }
        .footer { margin-top: 20px; padding: 10px; border-top: 1px solid #ddd; font-size: 9px; color: #666; text-align: center; }
        .col-prio{ width:10%; } .col-type{ width:14%; } .col-title{ width:26%; } .col-message{ width:36%; } .col-action{ width:14%; }
...
<body>
    <div class="header">
        ...
        <div class="subtitle">SMART ALERTS</div>
        <div style="font-size:10px; line-height:1.5; margin: 6px 0 4px;">
            Priority-based alerts including out-of-stock, low stock, expiries and outstanding balances.
        </div>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    @php $p = strtolower($r['priority'] ?? ''); @endphp
                    <span class="badge {{ $p==='critical' ? 'badge-critical' : ($p==='high' ? 'badge-high' : 'badge-medium') }}">{{ strtoupper($r['priority'] ?? '') }}</span>
                </td>
