...
<head>
    <meta charset="UTF-8">
    <title>{{ $subshop->name ?? 'Shop' }} Suppliers Export</title>
    <style>
        ...
        .badge-success{ background:#4caf50; color:#fff; } .badge-secondary{ background:#607d8b; color:#fff; }
...
<body>
    <div class="header">
        ...
        <div class="subtitle">SUPPLIERS REPORT</div>
        <div style="font-size:10px; line-height:1.5; margin: 6px 0 4px;">
            This report lists suppliers with activity metrics (orders and total spent) based on your selected filters.
        </div>
        ...
        <tbody>
            ...
            @foreach ($rows as $e)
                <tr>
                    ...
                    <td>{{ $e->phone ?? '-' }}</td>
                    <td>
                        <span class="badge {{ $e->is_active ? 'badge-success' : 'badge-secondary' }}">{{ $e->is_active ? 'ACTIVE' : 'INACTIVE' }}</span>
                    </td>
