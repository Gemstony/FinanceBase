...
<head>
    <meta charset="UTF-8">
    <title>{{ $subshop->name ?? 'Shop' }} Transactions Export</title>
    <style>
        ...
        .col-no{ width:4%; } .col-date{ width:12%; } .col-ord{ width:10%; } .col-cust{ width:15%; }
...
<body>
    <div class="header">
        ...
        <div class="subtitle">SALES TRANSACTIONS REPORT</div>
        <div style="font-size:10px; line-height:1.5; margin: 6px 0 4px;">
            This document provides a detailed view of all payment transactions including payments and refunds for the selected period and filters.
        </div>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ optional($txn->order)->order_no ?? '-' }}</td>
                <td>{{ optional($txn->order->customer)->name ?? '-' }}</td>
                <td>
