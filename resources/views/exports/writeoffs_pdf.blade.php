...
<head>
    <meta charset="utf-8">
    <title>{{ $subshop->name ?? 'Shop' }} Write-offs Export</title>
    <style>
        ...
        .col-no{ width:4%; } .col-date{ width:12%; } .col-prod{ width:24%; } .col-reason{ width:10%; }
        .col-qty{ width:6%; } .col-num{ width:10%; } .col-status{ width:10%; } .col-user{ width:14%; }
...
<body>
    <div class="header">
        ...
        <div class="subtitle">WRITE-OFFS REPORT</div>
        <div style="font-size:10px; line-height:1.5; margin: 6px 0 4px;">
            This report summarizes inventory write-offs for the selected period and filters, including totals and
            status breakdowns for auditing and review.
        </div>
    ...
    <table class="stats-table">
        <tr>
            <td class="stat-blue">
                ...
                <div class="stat-value">{{ number_format($summary['count'] ?? ($rows->count() ?? 0)) }}</div>
            </td>
            <td class="stat-green">
                <div class="stat-label">Total Value</div>
                <div class="stat-value">TZS{{ number_format($summary['total_value'] ?? 0, 2) }}</div>
            </td>
        </tr>
        <tr>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th class="col-num right">UNIT PRICE</th>
                <th class="col-num right">TOTAL VALUE</th>
        ...
        <tbody>
            ...
            @foreach ($rows as $e)
                <tr>
                    <td>{{ $i++ }}</td>
                    <td>{{ optional($e->write_off_date)->format('Y-m-d') }}</td>
                    <td>{{ $e->product->name ?? '-' }}</td>
                    <td>{{ ucfirst($e->reason) }}</td>
                    <td class="right">{{ (int) $e->quantity }}</td>
                    <td class="right">{{ number_format((float) $e->unit_price, 2) }}</td>
                    <td class="right">{{ number_format((float) $e->total_value, 2) }}</td>
                    <td>
