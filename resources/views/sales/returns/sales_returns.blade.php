...
@section('title', 'Sales Returns - ' . ($subshop->name ?? ''))

@section('content_header')
    <div class="card" style="background: var(--sidebar-bg); color: white; border: none; margin-bottom: 20px;">
        <div class="card-body d-flex justify-content-between align-items-center">
            <div>
                <h1 class="d-none d-md-block text-light"><i class="fas fa-undo-alt"></i> Sales Returns</h1>
                <h1 class="d-md-none text-light"><i class="fas fa-undo-alt"></i> Returns</h1>
                <div class="small text-light-50">Shop: {{ $subshop->name ?? '-' }}</div>
            </div>
            <a href="{{ route('invoices.index') }}" class="btn btn-outline-light btn-sm"><i class="fas fa-history"></i> Sales History</a>
        </div>
    </div>
@stop
        ...
        <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <div class="btn-group">
                    ...
                    @endphp
                    @can('export_sales_returns')
                    <button type="button" class="btn btn-outline-secondary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <i class="fas fa-file-export"></i> Export
                    </button>
                    <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item" href="{{ route('returns.export', array_merge(['format' => 'csv'], $params)) }}"><i class="fas fa-file-csv mr-2"></i>CSV</a>
                        <a class="dropdown-item" href="{{ route('returns.export', array_merge(['format' => 'excel'], $params)) }}"><i class="fas fa-file-excel mr-2"></i>Excel</a>
                        <a class="dropdown-item" href="{{ route('returns.export', array_merge(['format' => 'pdf'], $params)) }}"><i class="fas fa-file-pdf mr-2"></i>PDF</a>
            ...
            <!-- Summary cards (AdminLTE info-box style) -->
            <div class="row mb-3">
                ...
                <div class="bg-light p-2 rounded border">
                    <div class="form-row align-items-end">
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <a class="btn btn-light border" href="{{ route('returns.index', ['subshop_id'=>$subshop->id]) }}"><i class="fas fa-undo"></i> Reset</a>
                        </div>
                    </div>
                </div>
            </form>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-sm" role="group">
                                    @can('print_sales_return')
                                    <a href="{{ route('returns.print', $r->id) }}" target="_blank" class="btn btn-outline-primary" title="View Return Receipt">
                                        <i class="fas fa-file-invoice"></i>
                                    </a>
                                    <button type="button" class="btn btn-dark escpos-return" data-id="{{ $r->id }}" title="ESC/POS Print">
                                        <i class="fas fa-print"></i>
                                    </button>
                                    @endcan
                                    {{-- @can('restore_and_delete_sales_return') --}}
                                    <form method="POST" action="{{ route('returns.destroy', $r->id) }}" class="d-inline delete-return-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-outline-warning" title="Delete and Restore the Returns">
...
<script>
// Global JS config for this page
window.DUKA = Object.assign({}, window.DUKA || {}, {
    apiReturnsBase: @json(url('/api/returns')),
...
$(function(){
    ...
    $(document).on('submit', '.delete-return-form', function(e){
        ...
        if (window.Swal) {
            Swal.fire({
                title: 'Delete & Restore sales return?',
                ...
                if (result.isConfirmed) {
                    // Show loading while processing delete & restore
                    if (window.Swal) {
                        Swal.fire({
                            title: 'Processing...',
                    ...
                    fetch(action, {
                        ...
                        if (row && row.length) {
                            const dtApi = $('#ReturnsTable').DataTable();
                            dtApi.row(row).remove().draw(false);
                        }
                        Swal.fire({ icon:'success', title:'Success', text: 'Sales return deleted and restored successfully', timer:1200, timerProgressBar: true, showConfirmButton:false });
                    }).catch(err => {
            ...
            if (confirm('Delete this sales return? This will restore the item to the order and adjust totals.')) {
                fetch(action, {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json',
...
<script>
// ESC/POS Sales Return printing handler (delegated)
document.addEventListener('click', function(e){
    const btn = e.target.closest('.escpos-return');
    if(!btn) return;
    e.preventDefault();
    const id = btn.getAttribute('data-id');
    const base = (window.DUKA && window.DUKA.apiReturnsBase) ? window.DUKA.apiReturnsBase : '';
            ...
            if (result.isConfirmed || result.isDenied) {
                ...
                try {
                    ...
                    const res = await fetch(apiUrl, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': (window.DUKA && window.DUKA.csrf) ? window.DUKA.csrf : '' },
                    ...
                    if (data.dummy && data.data) {
                        ...
                        const a = document.createElement('a'); a.href = url; a.download = 'sales-return-'+id+'-escpos.bin'; document.body.appendChild(a); a.click(); a.remove();
